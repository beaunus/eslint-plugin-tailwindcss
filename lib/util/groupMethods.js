...
function generateOptions(propName, keys, config, isNegative = false) {
  ...
  switch (propName) {
    ...
    case 'outline':
      ...
      const namedGroup = isNegative ? '?<negativeValue>' : '?<value>';
      return '(' + namedGroup + keys.join('|') + ')';
    case 'fill':
      if (supportArbitrary) {
        // All units
        keys.push(length.mergedUnitsRegEx);
...
function parseClassname(name, arr, config, index = null) {
  ...
  const findInGroup = (name, group, parentType = null) => {
    if (typeof group.members === 'string') {
      ...
      const innerGroup = group.members.find((v) => findInGroup(name, v, group.type));
      if (!innerGroup) {
        return null;
      } else {
        return findInGroup(name, innerGroup, group.type);
      }
    }
  };
  let slot = null;
  arr.forEach((group) => {
    if (slot === null) {
      const found = findInGroup(classSuffix, group);
      if (found) {
        slot = found;
      }
    }
  ...
  return {
    index: index,
    name: name,
    variants: classPrefixes,
    type: slot ? slot.group : '',
